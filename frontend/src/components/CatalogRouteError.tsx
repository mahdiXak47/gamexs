...
export default function CatalogRouteError({
  error,
  unstable_retry,
  title = "این فهرست موقتاً در دسترس نیست",
  description = "دریافت اطلاعات با خطا روبه‌رو شد. دوباره تلاش کنید یا به فهرست همه بازی‌ها برگردید.",
}: {
        ...
        <section className="ps-header text-white">
          <div className="mx-auto grid min-h-[calc(100dvh-360px)] max-w-7xl content-center gap-8 px-4 py-16 sm:px-6">
            <div className="max-w-2xl">
              ...
              <p className="mt-5 max-w-xl text-sm leading-8 text-blue-100 sm:text-base">{description}</p>
              {error.digest && (
