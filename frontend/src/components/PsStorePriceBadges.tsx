...
function PriceCard({ tooltip, hero, children }: CardProps) {
  return (
    <div className={`group relative flex flex-col items-center cursor-default select-none transition-colors
      ${hero
    ...
    >
      ...
      {/* Tooltip — slides up on hover, RTL Persian text */}
      <div
        dir="rtl"
        className="
          pointer-events-none absolute bottom-full mb-2 right-0 left-0 mx-auto w-max max-w-[220px]
...
export default function PsStorePriceBadges({ info, hero }: Props) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      {/* Turkey price */}
      <PriceCard tooltip={trTooltip} hero={hero}>
      ...
      {/* US price */}
      <PriceCard tooltip={usTooltip} hero={hero}>
        <svg width="32" height="21" viewBox="0 0 36 24" xmlns="http://www.w3.org/2000/svg" aria-label="آمریکا" className="rounded-sm overflow-hidden shadow-sm">
          ...
          {[1,2,3,4,5,6].map((row) =>
            [1,2,3,4,5,6,7,8,9].slice(0, row % 2 === 0 ? 8 : 9).map((col, ci) => (
